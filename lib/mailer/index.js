...
const sendMail = async (template, subject, email, emailData, from = process.env.FROM_MAIL) => {
    ...
    locals.contactMail = process.env.ADMIN_CONTACT_MAIL || '';
    locals.site_url = process.env.SITE_URL || '';

    const renderedTemplate = await templateRenderer.render(template, emailData);
