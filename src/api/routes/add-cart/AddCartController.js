...
class UserController {
    ...
    try {
      ...
      if (!result) {
        ...
        await AddCart.updateOne(
          ...
          { $push: { cart_item: req.body.cart_item } }
        );
  ...
  async delete(req, res, next) {
    ...
    let cartResult = await AddCart.findOne({
      user_id: req.body.user_id,
      cart_item: {
        $elemMatch: { dish_id: mongoose.Types.ObjectId(req.body.dish_id) },
      },
    });
    ...
    let updatecart = await AddCart.updateOne(
      { user_id: req.body.user_id },
      {
        $pull: {
          cart_item: { dish_id: mongoose.Types.ObjectId(req.body.dish_id) },
        },
      }
    );
  ...
  async update(req, res, next) {
    ...
    let updatecart = await AddCart.updateOne(
      ...
      { $set: { "cart_item.$.quantity": req.body.quantity } }
    );
