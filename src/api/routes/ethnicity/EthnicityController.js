const {
  ...
  class UserController {
	async create(req, res, next) {
	  ...
	  try {
		...
		return newRecord
		  .save()
		  .then((results) => {
			return res.success(results, req.__("ETHNICITY_CREATE_SUCCESSFULLY"));
		  })
		  .catch((err) => {
	  ...
	  asyncParallel(
		{
		  ...
		  records_filtered: function(callback) {
			Ethnicity.countDocuments(conditions, (err, result) => {
			  /* send success response */
			  callback(err, result);
			});
		  },
		  records_total: function(callback) {
			Ethnicity.countDocuments({ is_deleted: 0 }, (err, result) => {
			  /* send success response */
			  callback(err, result);
			});
		  },
		...
		function(err, results) {
		  ...
		  return res.success(data, req.__("ETHNICITY_LIST_GENREATED"));
		}
	  );
	}
	...
	async detail(req, res, next) {
	  if (!req.params.slug) {
		return res.notFound(
		  ...
		  req.__("ETHNICITY_NOT_EXIST")
		);
	  }
  
	  try {
		let data = await Ethnicity.findOne(
		  {
		...
		if (data == null) return res.notFound({}, req.__("ETHNICITY_NOT_EXIST"));
  
		return res.success(data, req.__("ETHNICITY_DETAIL_SUCCESSFULLY"));
	  } catch (err) {
		return res.json({ data: err });
	  }
	}
	...
	async delete(req, res, next) {
	  ...
	  try {
		let data = await Ethnicity.updateOne(
		  {
			slug: req.params.slug,
		  },
		  { is_deleted: 1 }
		...
		return res.success(data, req.__("ETHNICITY_DELETE_SUCCESSFULLY"));
	  } catch (err) {
		return res.json({ data: err });
	  }
	}
	...
	async UpdateStatus(req, res, next) {
	  ...
	  try {
		...
		let updatedData = await Ethnicity.updateOne(
		  {
			slug: req.params.slug,
		  },
		  {
		...
		return res.success(data, req.__("ETHNICITY_STATUS_UPDATE_SUCCESSFULLY"));
	  } catch (err) {
	...
	async update(req, res, next) {
	  ...
	  try {
		...
		await Ethnicity.findOneAndUpdate({ slug: req.params.slug }, { ...data });
  
		return res.success(data, req.__("ETHNICITY_UPDATE_SUCCESSFULLY"));
	  } catch (err) {
		return res.json({ data: err });
	  }
	}
	...
	async listAll(req, res, next) {
	  ...
	  asyncParallel(
		...
		function(err, results) {
		  ...
		  return res.success(data, req.__("ethnicity_LIST_DONE"));
		}
	  );
	}
  
	async getAdminSetting(req, res) {
	  let adminSetting = await Ethnicity.findOne();
	  const userJson = {};
	  if (adminSetting) {
