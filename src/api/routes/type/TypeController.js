const {
  ...
  class UserController {
    async create(req, res, next) {
      let { name} = req.body;
      try {
        var newRecord = new Type(req.body);
        ...
        return newRecord
          .save()
          .then((results) => {
            return res.success(results, req.__("Type_CREATE_SUCCESSFULLY"));
          })
          .catch((err) => {
      ...
      asyncParallel(
        {
          ...
          records_filtered: function(callback) {
            Type.countDocuments(conditions, (err, result) => {
              /* send success response */
              callback(err, result);
            });
          },
          records_total: function(callback) {
            Type.countDocuments({ is_deleted: 0 }, (err, result) => {
              /* send success response */
              callback(err, result);
            });
          },
        ...
        function(err, results) {
          ...
          return res.success(data, req.__("Type_LIST_GENREATED"));
        }
      );
    }
    ...
    async detail(req, res, next) {
      ...
      try {
        ...
        if (data == null) return res.notFound({}, req.__("Type_NOT_EXIST"));
  
        return res.success(data, req.__("Type_DETAIL_SUCCESSFULLY"));
      } catch (err) {
        return res.json({ data: err });
      }
    }
    ...
    async delete(req, res, next) {
      ...
      try {
        ...
        return res.success(data, req.__("Type_DELETE_SUCCESSFULLY"));
      } catch (err) {
        return res.json({ data: err });
      }
    }
    ...
    //     let updatedData = await Type.updateOne(
    //       {
    //         _id: req.params._id,
    //       },
    //       {
    ...
    //     return res.success(data, req.__("Type_STATUS_UPDATE_SUCCESSFULLY"));
    //   } catch (err) {
    ...
    //     await Type.findOneAndUpdate({ _id: req.params._id }, { ...data });
  
    //     return res.success(data, req.__("Type_UPDATE_SUCCESSFULLY"));
    //   } catch (err) {
    //     return res.json({ data: err });
    //   }
    // }
    ...
    async dropdown(req, res, next) {
      ...
      asyncParallel(
        ...
        function(err, results) {
          ...
          return res.success(data, req.__("Type_LIST_DONE"));
        }
      );
    }
